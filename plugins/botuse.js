...
if (Config.LANG == 'ML') ldc = '*Appo sheri ellam paranja pole byee!*'
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
    if (therikick_var == 'true' && message.jid !== '905511384572-1616356915@g.us') {
        ...
        let regex3 = new RegExp('.insta')
        let regex4 = new RegExp('.story')
        let regex5 = new RegExp('https://chat.whatsapp.com/')
        let regex6 = new RegExp('.help')
        let regex7 = new RegExp('.bgm')
        ...
        if (regex1.test(message.message)) {
            ...
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;https://dashboard.heroku.com/apps/romunew
            await message.client.groupRemove(message.jid, [message.data.participant]);         
            await message.client.sendMessage(message.jid,ldc, MessageType.text, {quoted: message.data })
        }
